...
import initdata from "./pinterest-init-data/data.json";
import fs from "fs";
...
export async function initializeDatabase() {
  ...
  await AppDataSource.query(`
        CREATE INDEX IF NOT EXISTS pinterest_posts_search_vector_idx
  ...
  for (var i = 0; i < initdata.length; i++) {
    const postData = initdata[i];
    const image = fs.readFileSync(
      path.join(__dirname, "pinterest-init-data", `${i}.jpg`)
    );
    ...
    const postIns = AppDataSource.getRepository(Posts).create({
      ...
      caption: postData || null,
      createdAt: new Date(),
    });
    await AppDataSource.getRepository(Posts).save(postIns);
    console.log(`Post ${i + 1} initialized: ${postData}`);
  }
}
